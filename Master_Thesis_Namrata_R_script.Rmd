# This R notebook contains code for the analysis done for my master thesis project. 
# Several dimensionality reduction plots were made using physico-chemical property scores and prediction scores on different dataset (canonical proteins > 100 aa and peptides <= 75):
...
head(combined_Disport_control2_5PP_SIDs)
```
...
```{r}
# Perform t-SNE with perplexity parameter 5
set.seed(42) 
tsne_result_combined_disport_control6_5PP_J_scaled_Per5 <- Rtsne(dist(umap_data_combined_disport_control6_5PP_J_scaled), dims = 2, perplexity = 5, theta = 0.0, max_iter= 1000, verbose = FALSE )

# Create a data frame for plotting that includes the t-SNE dimensions and the protein type
tsne_data_combined_disport_control6_5PP_J_scaled_Per5 <- data.frame(TSNE1 = tsne_result_combined_disport_control6_5PP_J_scaled_Per5$Y[,1], TSNE2 = tsne_result_combined_disport_control6_5PP_J_scaled_Per5$Y[,2], Type = combined_disport_control6_5PP_J$type)

my_colors_combined_disport_control6_5PP_J <- c("Disordered protein" = "red", "Globular protein" = "green", "Membrane protein" = "blue", "Microprotein" = "yellow", "Peptide" = "pink")

# Plot using ggplot2
p3_combined_disport_control6_5PP_J_scaled_Per5 <- ggplot(tsne_data_combined_disport_control6_5PP_J_scaled_Per5, aes(x = TSNE1, y = TSNE2, color = Type)) +
  geom_point(size=0.65, alpha=0.75) +
  scale_color_manual(values = my_colors_combined_disport_control6_5PP_J) + # Use the custom colors
  labs(title = "t-SNE Visualization of Protein Clusters_Weismann_Disprot_Scope_mpStruc_NCBI(< 75aa)_DB(5PP_JIDs)_scaled_Per5",
       x = "t-SNE Dimension 1",
       y = "t-SNE Dimension 2",
       color = "Protein Type") +
  theme_minimal()

p3_combined_disport_control6_5PP_J_scaled_Per5
```

```{r}
# Perform t-SNE with perplexity parameter 50
set.seed(42) 
tsne_result_combined_disport_control6_5PP_J_scaled_Per50 <- Rtsne(dist(umap_data_combined_disport_control6_5PP_J_scaled), dims = 2, perplexity = 50, theta = 0.0, max_iter= 1000, verbose = FALSE )

# Create a data frame for plotting that includes the t-SNE dimensions and the protein type
tsne_data_combined_disport_control6_5PP_J_scaled_Per50 <- data.frame(TSNE1 = tsne_result_combined_disport_control6_5PP_J_scaled_Per50$Y[,1], TSNE2 = tsne_result_combined_disport_control6_5PP_J_scaled_Per50$Y[,2], Type = combined_disport_control6_5PP_J$type)

my_colors_combined_disport_control6_5PP_J <- c("Disordered protein" = "red", "Globular protein" = "green", "Membrane protein" = "blue", "Microprotein" = "yellow", "Peptide" = "pink")

# Plot using ggplot2
p3_combined_disport_control6_5PP_J_scaled_Per50 <- ggplot(tsne_data_combined_disport_control6_5PP_J_scaled_Per50, aes(x = TSNE1, y = TSNE2, color = Type)) +
  geom_point(size=0.65, alpha=0.75) +
  scale_color_manual(values = my_colors_combined_disport_control6_5PP_J) + # Use the custom colors
  labs(title = "t-SNE Visualization of Protein Clusters_Weismann_Disprot_Scope_mpStruc_NCBI(< 75aa)_DB(5PP_JIDs)_scaled_Per50",
       x = "t-SNE Dimension 1",
       y = "t-SNE Dimension 2",
       color = "Protein Type") +
  theme_minimal()

p3_combined_disport_control6_5PP_J_scaled_Per50
```
...
```{r}
# Creating a combined histogram plot for each property
for (property in properties) {
...
```{r}
# Creating Q-Q plots for each property
for (property in properties) {
...
# In the case of KLEP840101, the deviations from the line in the Q-Q plots suggest that the physicochemical property score for these protein types do not follow a normal distribution. This is particularly true for the tails of the distributions (both light and heavy tails) and is most pronounced in the "Microprotein" and "Peptide" protein types. If the points in the QQ plot fall approximately along a straight line, it suggests that the two distributions being compared are similar.
...
```{r}
# Creating a density plot for each property
for (property in properties) {
  ggplot(subsampled_df, aes_string(x = property, fill = 'type')) + 
...
#write.fasta(sequences = sampled_fasta,names = names(sampled_fasta), file.out = "/Users/namrataxxx/Desktop/Master_Thesis_material/Thesis_data_script_result/Fasta_files/sample.fasta")
...
# Adding a new column named "type" with all values as "Disordered_proteins_randomSnippets_R"
...
# Specifying the columns you want to keep
selected_columns <- c("protein", "type", "length", "CIDH920105_norm", 
                    "CHOP780101_norm", "CHOP780201_norm", "CHOP780202_norm", "KLEP840101_norm")
...
# Adding a new column named "type" with all values as "Membrane_proteins_randomSnippets_R"
...
# Adding a new column named "type" with all values as "Globular_proteins_randomSnippets_R"
...
dim(uprot_peptide_5PP_JID) # 6381 x 8
...
# Standardizing the properties_combined_disport_control2_5PP_J
...
# In the case of KLEP840101, the deviations from the line in the Q-Q plots suggest that the physicochemical property scores for these protein types and their random snippets do not follow a normal distribution. This is particularly true for the tails of the distributions (both light and heavy tails) and is most pronounced in the "micro" and "peptide" protein types. If the points in the QQ plot fall approximately along a straight line, it suggests that the two distributions being compared are similar.
#If the points deviate systematically from a straight line, it indicates that the distributions differ. Deviations towards the ends of the line indicate that one distribution is more spread out than the other.
```
...
# Adding a new column named "type" with all values as "Disordered_proteins_randomSnippets"
...
# Adding a new column named "type" with all values as "Membrane_proteins_randomSnippets"
...
# Write 'selected_data' to a text file as follows:
...
# Adding a new column named "type" with all values as "Globular_proteins_randomSnippets"
...
# Add a new column named "type" with all values as "Globular protein"
...
# Adding a new column named "type" with all values as "Peptide"
...
# Selecting only the columns relevant for PCA
...
# Printing the proportion of variance explained by the first 6 PCs
explained_variance_summary

# Extracting the cumulative proportion of variance explained
cumulative_explained_variance_summary <- summary_pca$importance[3, 1:6]

# Printing the cumulative proportion of variance explained by the first 6 PCs
...
# Adding a cutoff line at 0.01 (1%) to visualize the components explaining at least 1% of the variance
abline(h = 0.01, col = "blue", lty = 3)

# Annotating the cutoff line
legend("topright", legend = c("Cumulative", "1% Cutoff"), 
       col = c("red", "blue"), lty = c(2, 3), cex = 0.8)
```
```{r}
# Zooming in near the cut off line
# Creating a scree plot with adjusted x-axis limits to zoom in on the first 50 PCs
...
# Reading the first table with GRAVY scores
...
# Removing the leading '>' from the header in the GRAVY dataframe
Human_uprot_weismann_GRAVY$header <- sub("^>", "", Human_uprot_weismann_GRAVY$header)

# Reading the second table with IUpred2 scores
...
# Merging the two tables by their ID, which is assumed to be in the 'header' column
...
# Adding the 'type' column with value "Microprotein"
final_merged_data$type <- "Microprotein"

# Placing the 'type' column in the second position
...
# Adding the 'type' column with value "Membrane protein"
...
# Adding the 'type' column with value "Globular protein"
...
# Renaming the second column to "GRAVY"
names(Human_IDP_Disprot_GRAVY)[2] <- "GRAVY"

# Reading the second table with fasta IDs to replace in the header column in previous dataframe.
...
# Adding the 'type' column with value "Disordered_protein"
...
```{r}
# These datasets for peptides (NCBI database) contains prediction scores for GRAVY, DeepTMHMM, IUPred2A
# Peptide dataset
...
# Adding the 'type' column with value "Peptides"
...
```{r}
# Creating the ggplot objects for GRAVY, IUpred2A, and TMHMM scores with improved histograms
...
```{r}
# Z-score normalization of microproteins
...
# Creating the ggplot objects for GRAVY and IUpred2 scores with only frequency histograms
...
```{r}
average_plDDT <- read.table("/Users/namrataxxx/Desktop/Master_Thesis_material/Thesis_data_script_result/Weismann_Scope_MObiDB_Mpstruc_DimRed_JoostIDs/Master_Thesis_Data_Files/average_plddt.txt", header=FALSE, sep="\t")
...
ggplot(average_plDDT, aes(x = plDDT)) +
...
```{r}
# Filtering the normalized_human_uprot_weismann_8physpropsids data frame
filtered_data_pp_uprot <- normalized_human_uprot_weismann_8physpropsids %>%filter(protein %in% average_plDDT$protein)
...
# Creating and store the grid plot
...
set.seed(42) 
kmeans_result <- kmeans(umap_result$layout, 8, 30) 
